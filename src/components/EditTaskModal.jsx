    ...
    <div className="fixed inset-0  flex items-center justify-center w-screen h-screen m-auto rounded-md z-50 bg-primary-400/52">
      {isEdit ? (
        ...
        <form
          ...
          className={`${
            isDarkMode
              ? " max-h-fit md:overflow-x-hidden md:overflow-y-hidden overflow-y-scroll relative flex flex-col gap-5 items-start w-[343px] md:w-[30rem] px-[1.3rem] py-4  m-auto rounded-md bg-primary-500"
              : " max-h-fit flex flex-col gap-5 items-start w-[343px] md:w-[30rem]  px-[1.3rem] py-4  m-auto rounded-md bg-white"
          }`}
        >
          <section key={title} className="flex flex-col gap-[2rem] w-full">
            <div className="flex flex-col gap-2">
              <div className="flex w-full min-h-[60px]    items-center justify-between">
                <h3
                  className={`${
                    isDarkMode
                      ? "text-[17px] font-plus-jakarta-sans font-bold text-white max-w-[274px] md:max-w-[26rem]"
                      : "text-[17px] font-plus-jakarta-sans font-bold text-black max-w-[274px]  md:max-w-[26rem]"
                  }`}
                >
                  {title}
                </h3>
                <EllipsisVerticalIcon
                  ...
                  onClick={(e) => {
                    dispatch(
                      openSideTaskModal({
                        x: e.target.closest("form").getBoundingClientRect()
                          .width,
                        y: e.target.closest("form").getBoundingClientRect()
                          .height,
                      })
                    );
                    console.log(
                      e.target.closest("form").getBoundingClientRect()
                    );
                  }}
              ...
              {description && (
                <div>
                  <h3 className="md:text-[13px] font-plus-jakarta-sans text-primary-600 font-medium leading-[23px] md:max-w-[26rem]">
                    {description}
                  </h3>
                </div>
              )}
